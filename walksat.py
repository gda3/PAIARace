...
def rnd_interpretation(formula):
...
def walksat(formula, assignment, max_tries = 1000, max_flips = 1000):
	interpretation = []
	for i in xrange(max_tries):
		interpretation = rnd_interpretation(formula)
...
def main() :
	if len(sys.argv) != 2: 
		print "Usage: "+ sys.argv[0] +" file.cnf"
		return None
	formula, nvars = parse( sys.argv[1] )
	solution = walksat( formula, [] )
	if solution :
